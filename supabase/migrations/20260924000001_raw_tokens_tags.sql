...
-- (previously read from raw_payload.cmc_details.tags / raw_payload.cr_details.tags)
-- ============================================================
...
SET tags = COALESCE(
  (
    SELECT jsonb_agg(lower(regexp_replace(trim(tag), '\s+', '-', 'g')))
    FROM jsonb_array_elements_text(raw_payload->'cr_details'->'tags') AS tag
  ),
  '[]'::jsonb
)
